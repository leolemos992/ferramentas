...
function TableDetail({ table, onSelectTable }: { table: TableData, onSelectTable: (tableName: string) => void }) {
  const [columnSearchTerm, setColumnSearchTerm] = useState("");

  const filteredFields = useMemo(() => {
    if (!columnSearchTerm) return table.fields;
...
export function DictionaryViewer({ tables }: DictionaryViewerProps) {
  ...
  const [relatedTable, setRelatedTable] = useState<TableData | null>(null);
  ...
  const handleSelectTable = (tableName: string) => {
    ...
    if (tableToSelect) {
      setRelatedTable(tableToSelect);
    }
  };
  
  const handleSelectPrimaryTable = (table: TableData) => {
    setSelectedTable(table);
    setRelatedTable(null); // Fecha o painel lateral ao selecionar uma nova tabela principal
  };
  ...
  return (
    <>
      <div className="flex h-[calc(100vh-22rem)] border rounded-lg bg-card text-card-foreground">
            ...
            <div className="p-2 space-y-1">
              {filteredTables.map((table) => (
                <Button
                  key={table.name}
                  variant="ghost"
                  onClick={() => handleSelectPrimaryTable(table)}
                  className={cn(
        ...
        <main className="w-3/4 p-4 md:p-6">
          <ScrollArea className="h-full pr-4">
              {selectedTable ? (
                  <TableDetail table={selectedTable} onSelectTable={handleSelectTable} />
              ) : (
      ...
      <Sheet open={!!relatedTable} onOpenChange={(open) => !open && setRelatedTable(null)}>
        <SheetContent className="w-full sm:max-w-2xl lg:max-w-4xl p-0">
          <div className="flex flex-col h-full">
            <SheetHeader className="p-6">
                <SheetTitle>Tabela Relacionada: {relatedTable?.name}</SheetTitle>
                <SheetDescription>{relatedTable?.description}</SheetDescription>
            </SheetHeader>
            <div className="flex-1 overflow-hidden">
                <ScrollArea className="h-full px-6 pb-6">
                  {relatedTable && <TableDetail table={relatedTable} onSelectTable={handleSelectTable} />}
                </ScrollArea>
            </div>
